  ...
  return (
    <div style={{ display: "flex", maxWidth: "80vw", margin: "24px auto", padding: "0 24px", gap: 24 }}>
      ...
      <div style={{ flex: 1 }}>
        ...
        {/* Statistics Overview - based on filtered data */}
        {filtered.length > 0 && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 16, marginBottom: 16 }}>
            ...
            >
              <div style={{ fontSize: 28, fontWeight: 700, color: "#3B82F6" }}>{filtered.length}</div>
              <div style={{ fontSize: 13, color: "#6B7280", marginTop: 4 }}>Datasets</div>
            </div>
            <div
              onClick={() => {}}
              style={{ background: "#fff", borderRadius: 12, padding: "16px 20px", border: "1px solid #E2E8F0", textAlign: "center", cursor: "not-allowed", opacity: 0.6 }}
            >
              <div style={{ fontSize: 28, fontWeight: 700, color: "#10B981" }}>-</div>
              <div style={{ fontSize: 13, color: "#6B7280", marginTop: 4 }}>Samples</div>
            </div>
            {new Set(filtered.map(d => d.species)).size > 0 && (
              <div
                onClick={() => speciesFilter.length ? setSpeciesFilter([]) : setSpeciesFilter([...new Set(filtered.map(d => d.species))])}
              ...
              >
                <div style={{ fontSize: 28, fontWeight: 700, color: "#8B5CF6" }}>{new Set(filtered.map(d => d.species)).size}</div>
                <div style={{ fontSize: 13, color: "#6B7280", marginTop: 4 }}>Species</div>
              </div>
            )}
            {new Set(filtered.map(d => d.tissue)).size > 0 && (
              <div
                onClick={() => tissueFilter.length ? setTissueFilter([]) : setTissueFilter([...new Set(filtered.map(d => d.tissue))])}
              ...
              >
                <div style={{ fontSize: 28, fontWeight: 700, color: "#F59E0B" }}>{new Set(filtered.map(d => d.tissue)).size}</div>
                <div style={{ fontSize: 13, color: "#6B7280", marginTop: 4 }}>Tissues</div>
              </div>
            )}
            {new Set(filtered.map(d => d.omicsType)).size > 0 && (
              <div
                onClick={() => omicsFilter.length ? setOmicsFilter([]) : setOmicsFilter([...new Set(filtered.map(d => d.omicsType))])}
              ...
              >
                <div style={{ fontSize: 28, fontWeight: 700, color: "#EF4444" }}>{new Set(filtered.map(d => d.omicsType)).size}</div>
