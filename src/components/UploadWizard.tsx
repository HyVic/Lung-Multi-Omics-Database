...
import { Upload, Button, Form, Input, Select, Radio, Modal } from "antd";
...
export default function UploadWizard({ onClose, onSubmit }: UploadWizardProps) {
  ...
  return (
    ...
    >
      <div style={{ display: "flex", flexDirection: "column", maxHeight: "calc(80vh - 120px)" }}>
        {/* Step Indicator */}
        ...
        <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
          ...
          {step === 2 && (
            <div>
              ...
              {/* scRNA-seq Upload Section - simplified, same as other types */}
              {data.omicsType === "scRNA-seq" && (
                <div style={{ border: "1px solid #E2E8F0", borderRadius: 12, padding: 20, marginBottom: 16 }}>
                  <div style={{ marginBottom: 20 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                      <span style={{ color: "#EF4444", fontSize: 12 }}>*</span>
                      <span style={{ fontSize: 13, fontWeight: 600, color: "#374151" }}>scRNA-seq Data File</span>
                      <span style={{ fontSize: 11, color: "#6B7280" }}>(required)</span>
                    </div>
                    <div style={{ fontSize: 11, color: "#6B7280", marginBottom: 8 }}>Format: .mtx / .tsv / .csv / 10X format (barcodes.tsv, features.tsv, matrix.mtx)</div>
                    <FileUploadZone
                      file={data.expressionFile}
              ...
              {!["bulk RNA-seq", "Total RNA-seq", "polyA RNA-seq", "scRNA-seq"].includes(data.omicsType) && (
                <div style={{ border: "1px solid #E2E8F0", borderRadius: 12, padding: 20, marginBottom: 16 }}>
                  <div style={{ marginBottom: 20 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                      <span style={{ color: "#EF4444", fontSize: 12 }}>*</span>
                      <span style={{ fontSize: 13, fontWeight: 600, color: "#374151" }}>{data.omicsType} Data File</span>
                      <span style={{ fontSize: 11, color: "#6B7280" }}>(required)</span>
                    </div>
                    <div style={{ fontSize: 11, color: "#6B7280", marginBottom: 8 }}>Format: .txt / .csv / .xlsx / .gz</div>
                    <FileUploadZone
                      file={data.expressionFile}
                      onFileSelect={(file) => updateData({ expressionFile: { name: file.name, data: file } })}
                      onClear={() => updateData({ expressionFile: undefined })}
                      acceptedTypes=".txt,.csv,.xlsx,.xls,.gz"
                    />
                  </div>
                </div>
              )}
