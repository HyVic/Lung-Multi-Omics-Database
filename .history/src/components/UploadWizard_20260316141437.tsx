...
export interface UploadData {
  ...
  citation?: string;
  // Step 2: Files
  files: { name: string; type: string; data: File }[];
        ...
        <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
          ...
          {step === 2 && (
            <div>
              ...
              {data.files.length > 0 && (
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 8 }}>Selected Files ({data.files.length})</div>
                  {data.files.map((file, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 12px", background: "#F8FAFC", borderRadius: 6, marginBottom: 6 }}>
