...
function FileUploadZone({ file, multiple, onFileSelect, onFilesSelect, onClear, acceptedTypes }: FileUploadZoneProps) {
  const inputId = useId();
    ...
    <div style={{ border: "2px dashed #D1D5DB", borderRadius: 8, padding: 16, textAlign: "center" }}>
      {!file ? (
        <>
          ...
          <label
            htmlFor={inputId}
            style={{ display: "inline-block", padding: "8px 16px", background: "#3B82F6", color: "#fff", borderRadius: 6, fontSize: 12, cursor: "pointer" }}
          >
            📂 Click to select file
          </label>
          <div style={{ fontSize: 11, color: "#6B7280", marginTop: 8 }}>Drag & drop supported</div>
        </>
      ) : (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "#F0FDF4", border: "1px solid #86EFAC", borderRadius: 6, padding: "8px 12px" }}>
          <span style={{ fontSize: 12, color: "#166534" }}>✓ {file.name}</span>
          <button
            onClick={onClear}
